...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "🚫 Unauthorized Access!🚫Please buy access :- @NxTLvLKaran"
...
def welcome_start(message):
    ...
    response = f'''🫡WELCOME TO NxTLvLMods Server Crasher BOT 🚀 @NxTLvLMods '''
    bot.reply_to(message, response)
...
def welcome_rules(message):
    user_name = message.from_user.first_name
    response = f'''
1. ANY PROBLEM DM HERE -> @NxTLvLKaran'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "⚠️ THIS MESSAGE SEND BY DDOS OWNER:@NxTLvLKaran\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
